...
class ArbiEncoderLSTM(nn.Module):
    def __init__(self, hidden_size=48, num_layer=3):
        ...
        self.hidden_size = hidden_size
        self.num_layer = num_layer
        self.encoder = nn.LSTM(2, self.hidden_size, self.num_layer, bidirectional=True)
...
class IntentionExtractor(nn.Module):
    ...
    def forward(self, hidden_state):
        Intention = self.generator(hidden_state)
...
class PCCSNet(nn.Module):
    def __init__(self, obs_len=2, pre_len=12, n_cluster=200, obs_hidden_size=48, pred_hidden_size=48, num_layer=3):
        super().__init__()
...
class PCCSNet_XOE(nn.Module):
    def __init__(self, args, num_labels=6, input_dim=5, num_layer=3):

        super().__init__()
        self.obs_len = args.obs_len
        self.pre_len = args.pred_len
        self.obs_hidden_size = args.obs_feat_size
        self.pred_hidden_size = args.pred_feat_size
        self.num_layer = num_layer

        self.encoder_obs = XOE(args, num_labels, input_dim)
        self.encoder_pre = EncoderLSTM(2, self.pred_hidden_size // 2, self.num_layer)
        self.synthesizer = Synthesizer(self.obs_hidden_size, self.pred_hidden_size)
        self.classifier = Classifier(self.obs_hidden_size, args.n_cluster)
        self.decoder = Decoder(self.pre_len, self.obs_hidden_size // 2, self.pred_hidden_size // 2, self.num_layer)
    ...
    def forward(self, obs_encoded, k=None, batch_labels=None, all_modalities=None, obs_feature_len=None):
        ...
        assert k <= len(prob_list)
        expanded_obs_encoding = obs_encoded.repeat(k, 1)
        predicted_future_encoding = self.synthesizer(expanded_obs_encoding, labels=prob_list[:k], all_modalities=all_modalities)
        output = self.decoder([expanded_obs_encoding, predicted_future_encoding])
        return output

    def gen_encoding(self, obs_trj, obs_data, data_sizes, trj_pred, seq_lens=None, for_training=True):
        bs, _, _ = obs_trj.shape

        with torch.no_grad():
            obs_encoding = self.encoder_obs(obs_trj, obs_data, data_sizes, seq_lens)
            if for_training:
                trj_pred = trj_pred.transpose(0, 1)
                encoder_pre_hidden = self.encoder_pre.initHidden(bs)
                pre_encoded, _ = self.encoder_pre(trj_pred, encoder_pre_hidden)
    ...
    def load_encoders(self, save_dir):
        checkpoint = torch.load(os.path.join(save_dir, "Obs_Encoder_XOE.pth"))
        self.encoder_obs.load_state_dict(checkpoint)
        checkpoint = torch.load(os.path.join(save_dir, "Pred_Encoder.pth"))
        self.encoder_pre.load_state_dict(checkpoint)
